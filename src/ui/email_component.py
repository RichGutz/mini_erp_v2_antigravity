...
def render_email_sender(key_suffix: str, documents: list):
    """
    ...
    with st.expander("✉️ Redactar Correo", expanded=True):
        ...
        to_email = c1.text_input("Destinatario", placeholder="cliente@empresa.com", key=f"to_{key_suffix}")
        subject = c2.text_input("Asunto", placeholder="Envío de Documentos - Operación XYZ", key=f"sub_{key_suffix}")
